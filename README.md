...
The appropriate morning or evening list is selected automatically.

Press the counter beside a dhikr after each repetition.

Continue until the counter reaches zero.

Use the reset button if a counter needs to be restarted.

Follow the progress indicator until the list is complete.

Use the completion action to add completed items to the current history.

Record a Tasbih Session

Open the tasbih section.

Enter the dhikr name or phrase.

Start the session.

Use the increment and decrement controls to update the count.

Optionally start and stop the timer.

Save the session to display it in the current history.

Export History

Complete at least one dhikr or save a tasbih session.

Open the history section.

Select the save/export action.

Choose a filename and location.

The application writes the visible records to a text file.
